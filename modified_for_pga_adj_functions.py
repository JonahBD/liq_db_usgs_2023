...
def soil_parameters(df, site):
    ...
    # df['qc calc'] = np.where(df['qc calc'] <= 0, float('NaN'), df['qc calc'])
    # df['qt calc'] = np.where(df['qt calc'] <= 0, float('NaN'), df['qt calc'])
...
def PGA_insertion(df, PGA_filepath, site):
    ...
    liq = pga_df.loc[site]['Liquefaction']
    df.at[0, 'Liquefaction'] = liq
    return df, eq, mag, liq
