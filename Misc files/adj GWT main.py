...
export_folder_path = r"C:\Users\jdundas2\OneDrive - Brigham Young University\Liq\Italy Data\Attempt 06 - GWT\Adj GWT\Soil Parameters"
    ...
    while keep_going:
        ...
        try:
            if counter == 0:
                ...
                df.at[0, 'GWT [m]'] = gwt
                df.at[1, "GWT [m]"] = 'OG GWT below'
                df.at[2, 'GWT [m]'] = og_gwt
        except KeyError:
        ...
        if gwt >= upper_bound or gwt <= (lower_bound + 0.001):
            ...
            df = df[[depth_column_name, 'qc (MPa)', 'fs (kPa)', 'u (kPa)', 'qt (MPa)', "Rf (%)",
                     ...
                     'OCR R', 'OCR K', 'cu_bq (kPa)', 'cu_14 (kPa)', 'cu_HB (kPa)', "M (kPa)", "k0_1", 'k0_2', "Vs R (m/s)", 'Vs M (m/s)', "k (m/s)", 'ψ',
                     "φ' R (degrees)",
                     ...
                     'za', 'zb',
                     "lat_wgs84","lon_wgs84", 'GWT [m]', 'Date of CPT [gg/mm/aa]', 'u [si/no]', 'preforo [m]', 'PGA', "EQ",
        ...
        if counter >= 300:
            keep_going = False
            print(f'Counter hit 300 so you have a problem on site {site}')
...
high_c_df = pd.DataFrame({'Site with lim gwt': pga_limited})
pga_values_c_50_df = pd.DataFrame({'GWT values': pga_values_hit_lim})
