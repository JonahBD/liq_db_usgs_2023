...
#################################################################
# Inputs
df = pd.read_excel(r"C:\Users\hf233\OneDrive - Brigham Young University\Liq\Gabrelle update\liq_param_compiled half stdv lower pga 6-3.xlsx" )
column_of_interest = 'LPI'
#################################################################

# Split the data into features (X) and target variable (y)
X = df[[column_of_interest]].values  # Features
...
plt.title(f'Receiver Operating Characteristic (ROC) Analysis for {column_of_interest}')
...
threshold_values = np.linspace(min(df[column_of_interest]), max(df[column_of_interest]), num=100)
...
for threshold in threshold_values:
    ...
    predictions = (df[column_of_interest] >= threshold).astype(int)
...
sns.kdeplot(liquefaction_present[column_of_interest], bw_adjust=0.5, label='Surficial Liquefaction Manifestation', color='black')

# KDE plot for cases where liquefaction is absent
sns.kdeplot(liquefaction_absent[column_of_interest], bw_adjust=0.5, label='No Surficial Liquefaction Manifestation', color='blue')

# Add labels and title
plt.xlabel(f'{column_of_interest}')
plt.ylabel('Frequency')
plt.title(f'Distribution of {column_of_interest} with respect to Liquefaction Manifestation')
