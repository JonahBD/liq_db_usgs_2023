...
################ USER INPUTS ############################
input_folder_path = r"C:\Users\jdundas2\OneDrive - Brigham Young University\Liq\Final Data\GWT Adj\GWT adj soil parameters"
export_folder_path = r"C:\Users\jdundas2\OneDrive - Brigham Young University\Liq\Final Data\GWT Adj"
#########################################################
...
for filename in glob.glob(os.path.join(input_folder_path, "*.xls*")):
    ...
    for site_characteristic in site_characteristics:
        if site_characteristic == "GWT [m]":
            if df.loc[1, site_characteristic] == "OG GWT below":
                liq_df.at[indx_counter, "OG GWT"] = df.loc[2, site_characteristic]
                liq_df.at[indx_counter, "New GWT"] = df.loc[0, site_characteristic]
                liq_df.at[indx_counter, "Diff GWT"] = df.loc[0, site_characteristic] - df.loc[2, site_characteristic]
                continue
            else:
                liq_df.at[indx_counter, "OG GWT"] = df.loc[0, site_characteristic]
                liq_df.at[indx_counter, "New GWT"] = float('Nan')
                liq_df.at[indx_counter, "Diff GWT"] = 0
                continue
    ...
    loop.update(1)
    if indx_counter == 30:
        liq_df.to_excel(f'{export_folder_path}\_30_liq_param_compiled_GWT_{date}.xlsx', index=False)
loop.close()

liq_df.to_excel(f'{export_folder_path}\liq_param_compiled_GWT_adj_{date}.xlsx', index=False)
